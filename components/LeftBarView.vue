...
import(`~/assets/json/2020/各黨數據.json`).then((res) => {
  ...
  voteData.value = importedPartyVote.value.reduce((acc, data) => {
...
<template>
  <div class="mt-2xl h-auto rounded-lg bg-white p-l sm:h-full sm:w-[270px] sm:space-y-l">
    <div class="relative flex items-center justify-between">
      <h6 class="font-bold text-inherit sm:font-semibold">投票概況</h6>
      <Icon name="fa-solid:chevron-down" width="55" :verticalFlip="isToggleArrow"
        class="w-[24px] opacity-100 sm:w-[55px] sm:opacity-0" />
      <span @click="isToggleArrow = !isToggleArrow"
        class="absolute bottom-[0px] left-[0px] right-[0px] top-[0px] block cursor-pointer sm:hidden"></span>
    </div>
    <section class="h-[0px] space-y-xl overflow-y-hidden transition-all duration-500 sm:h-auto sm:space-y-3xl"
      :class="{ 'mt-l h-[280px] ': isToggleArrow }">
      <div v-if="importedAllVote &&
        ...
        <div class="relative flex items-center justify-start gap-s sm:gap-l">
          <div class="!z-[1] h-[72px] w-[72px] border-none border-blue-800 sm:h-[120px] sm:w-[120px]">
            <Doughnut :data="chartConfig.dataS1" :options="chartConfig.optionsS1" :style="myStyles" />
          </div>
      ...
      <div v-if="importedPartyVote &&
        ...
        <div class="flex items-center justify-between gap-l">
          <div class="relative h-[72px] w-[72px] border-none border-blue-800 sm:h-[120px] sm:w-[120px]">
            <Doughnut :data="chartConfig.dataS2" :options="chartConfig.optionsS2" :style="myStyles" />
          </div>
        </div>
        <div class="space-y-l">
          <div class="h7 space-y-xs font-semibold" v-for="(data, index) in voteData" :key="index">
            <div class="flex gap-xxs sm:gap-s">
              <span class="h8 h-[24px] rounded-full pb-[3px] pl-[8px] pr-[9px] pt-[4px] text-white" :class="data.color">{{
                index + 1 }}</span>
              <div class="w-[104px] border-r-2 border-gray-400 pr-s">
                <p class="sm:h7 h8 font-normal sm:font-semibold">
                  {{ data.party }}
                </p>
                <p class="h8 flex items-center justify-between font-semibold child:flex-none">
